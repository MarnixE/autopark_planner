...
class RRT:
    ...
    def __init__(self,ox,oy,resolution=2,rr=4, bounds=np.array([0,1900]),
    ...
    def plan(self,sx,sy,gx,gy):
        ...
        for i in range(self.max_iter):
            rnd_node = self.get_random_node(self.node_list[-1])
            ...
            if self.dist_to_goal(self.node_list[-1].p) <= self.max_extend_length:
                ...
                if not self.collision(final_node, self.node_list[-1]):
                    return self.final_path(len(self.node_list) - 1),self.cost
        return None,self.cost  # cannot find path
    ...
    def get_random_node(self,node):
        ...
        if np.random.rand() > self.goal_sample_rate:
            ...
            rng = np.random.uniform(l_bound,u_bound,2)
            rng1 = int(rng[0])
            rng2 = int(rng[1])
            rnd = self.Node(np.array([rng1,rng2]))
        else:
            # Select goal point
...
class PathPlanning:
    def __init__(self,obstacles):
        self.margin = 5
        #sacale obstacles from env margin to path planning margin
        ...
        self.grid_size = 3
        self.robot_radius = 4
        self.rrt = RRT(self.ox, self.oy, self.grid_size, self.robot_radius)

    def plan_path(self,sx, sy, gx, gy):
        route,cost = self.rrt.plan(sx+self.margin, sy+self.margin, gx+self.margin, gy+self.margin)
        print(route)
        rx = []
        ry = []
        for i in route:
...
class ParkPathPlanning:
    ...
    def generate_park_scenario(self, sx, sy, gx, gy, path):
        computed_angle = angle_of_line(path[-2][0],path[-2][1],path[-1][0],path[-1][1])
